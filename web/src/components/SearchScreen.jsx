    ...
    <div style={{ height: '100%', overflowY: 'auto', paddingBottom: 8 }}>
      <div style={{ padding: '20px 20px 12px' }}>
        ...
        <div style={{
          ...
          background: colors.surfaceCard, borderRadius: 14, padding: '12px 16px',
          border: `1.5px solid ${query ? colors.primary : 'transparent'}`, transition: 'border 0.15s',
        }}>
          ...
          <input
            ...
            style={{
              ...
              color: colors.textPrimary, fontSize: 15, fontFamily: 'Inter, sans-serif',
            }}
          />
          {query ? (
            <button onClick={() => setQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.textMuted }}>
              <X size={18} />
            </button>
          ) : (
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.primary }}>
              <Mic size={18} />
            </button>
          )}
        </div>
      ...
      {query.length > 1 ? (
        <div>
          ...
          {filtered[0] && (
            <div style={{ padding: '0 20px 16px' }}>
              ...
              <div
                ...
                style={{ background: colors.surfaceCard, borderRadius: 16, padding: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ padding: '0 20px 4px' }}>
            <div style={{ color: colors.textPrimary, fontWeight: 800, fontSize: 16, marginBottom: 8 }}>Tracks</div>
          </div>
          {filtered.map((t, i) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {genres.map(g => (
              <div key={g.name} style={{
                borderRadius: 14, overflow: 'hidden', cursor: 'pointer', height: 80,
                background: `linear-gradient(135deg, ${g.color}dd, ${g.color}77)`,
                display: 'flex', alignItems: 'flex-end', padding: 14, position: 'relative',
              }}>
                <img src={`https://picsum.photos/seed/${g.seed}/100/100`} alt={g.name}
                  style={{ position: 'absolute', right: -8, top: -8, width: 70, height: 70, borderRadius: 10, objectFit: 'cover', opacity: 0.6, transform: 'rotate(20deg)' }} />
