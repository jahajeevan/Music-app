...
const genres = [
  { name: 'Afrobeats', color: '#00D4AA' }, { name: 'K-Pop', color: '#FD79A8' },
  { name: 'Amapiano', color: '#FFA502' }, { name: 'Drill', color: '#6C5CE7' },
  { name: 'Synthwave', color: '#74B9FF' }, { name: 'Reggaeton', color: '#E17055' },
]
const newFriday = [
...
export default function DiscoverScreen({ onPlayTrack, currentTrack }) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', paddingBottom: 8 }}>
      <div style={{ padding: '20px 20px 8px' }}><div style={{ color: colors.textPrimary, fontWeight: 900, fontSize: 22 }}>Discover</div></div>
      <div style={{ margin: '12px 20px', padding: '16px', background: colors.surfaceCard, borderRadius: 20 }}>
        ...
        {chartTracks.map(t => (
          <div key={t.id} onClick={() => onPlayTrack(t)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `1px solid ${colors.surfaceOverlay}`, cursor: 'pointer' }}>
            ...
            <img src={`https://picsum.photos/seed/${t.cover}/80/80`} alt={t.title} style={{ width: 36, height: 36, borderRadius: 6, objectFit: 'cover' }} />
      ...
      <div style={{ padding: '0 20px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 }}>
          {countries.map((flag, i) => (
            <button key={i} onClick={() => setSelectedCountry(i)} style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '10px 14px', borderRadius: 14, background: selectedCountry === i ? colors.primaryDim : colors.surfaceCard, border: `1.5px solid ${selectedCountry === i ? colors.primary : 'transparent'}`, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ display: 'flex', gap: 12, overflowX: 'auto' }}>
          {newFriday.map(a => (
            <div key={a.seed} style={{ flexShrink: 0, cursor: 'pointer' }} onClick={() => onPlayTrack(tracks[0])}>
              <img src={`https://picsum.photos/seed/${a.seed}/200/200`} alt={a.title} style={{ width: 130, height: 130, borderRadius: 14, objectFit: 'cover', display: 'block' }} />
      ...
      <div style={{ padding: '8px 20px 12px' }}>
        ...
        {tracks.slice(2, 5).map((t, i) => (<TrackRow key={t.id} track={t} index={i} onPlay={onPlayTrack} isActive={currentTrack?.id === t.id} />))}
      </div>
      <div style={{ padding: '8px 20px 24px' }}>
        <div style={{ color: colors.textPrimary, fontWeight: 800, fontSize: 16, marginBottom: 12 }}>Genre Deep Dives</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {genres.map(g => (<button key={g.name} style={{ padding: '8px 18px', borderRadius: 20, background: g.color + '22', border: `1.5px solid ${g.color}55`, color: g.color, fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>{g.name}</button>))}
        </div>
      </div>
    </div>
  )
