  ...
  return (
    <div style={{ overflowY: 'auto', height: '100%', paddingBottom: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 20px 12px' }}>
        ...
        <img src="https://picsum.photos/seed/user1/80/80" alt="avatar" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover' }} />
      </div>
      <div style={{ display: 'flex', gap: 8, padding: '0 20px 20px', overflowX: 'auto', paddingBottom: 4 }}>
        {moods.map(mood => {
          ...
          return (
            <button key={mood} onClick={() => setSelectedMood(mood)} style={{ flexShrink: 0, padding: '7px 16px', borderRadius: 20, background: sel ? c + '22' : colors.surfaceCard, border: `1.5px solid ${sel ? c : 'transparent'}`, color: sel ? c : colors.textSecondary, fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'Inter, sans-serif' }}>{mood}</button>
          )
        })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, padding: '12px 20px 20px', overflowX: 'auto' }}>
        {recentAlbums.map(a => (
          <div key={a.seed} style={{ flexShrink: 0, cursor: 'pointer' }} onClick={() => onPlayTrack(tracks[0])}>
            <img src={`https://picsum.photos/seed/${a.seed}/200/200`} alt={a.title} style={{ width: 120, height: 120, borderRadius: 14, objectFit: 'cover', display: 'block' }} />
      ...
      <div style={{ display: 'flex', gap: 12, padding: '0 20px 20px', overflowX: 'auto' }}>
        {newReleases.map(a => (
          <div key={a.seed} style={{ flexShrink: 0, cursor: 'pointer' }} onClick={() => onPlayTrack(tracks[1])}>
            <img src={`https://picsum.photos/seed/${a.seed}/200/200`} alt={a.title} style={{ width: 140, height: 140, borderRadius: 14, objectFit: 'cover', display: 'block' }} />
      ...
      <div style={{ display: 'flex', gap: 16, padding: '0 20px 24px', overflowX: 'auto' }}>
        {artists.map(a => (
          <div key={a.id} style={{ flexShrink: 0, textAlign: 'center', cursor: 'pointer' }}>
            <img src={`https://picsum.photos/seed/${a.seed}/200/200`} alt={a.name} style={{ width: 76, height: 76, borderRadius: '50%', objectFit: 'cover', display: 'block', border: `2px solid ${colors.surfaceOverlay}` }} />
