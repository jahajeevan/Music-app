...
import { ChevronDown, Heart, MoreHorizontal, SkipBack, SkipForward, Play, Pause,
  Shuffle, Repeat, Mic2, List, Waves, Plus } from 'lucide-react'
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', padding: '0 24px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0' }}>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.textPrimary, padding: 4 }}>
            <ChevronDown size={28} />
          </button>
          ...
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.textPrimary, padding: 4 }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px 0' }}>
          <div style={{
            width: 'min(280px, 70vw)', aspectRatio: '1',
            borderRadius: 20, overflow: 'hidden',
            boxShadow: isPlaying ? `0 0 60px ${colors.primary}55, 0 20px 60px rgba(0,0,0,0.6)` : '0 20px 60px rgba(0,0,0,0.6)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: colors.textPrimary, fontSize: 22, fontWeight: 900, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {track.title}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 12 }}>
            {track.quality && (
              <span style={{
                background: track.quality === 'HI-RES' ? colors.qualityHiRes + '22' : colors.accent + '22',
                color: track.quality === 'HI-RES' ? colors.qualityHiRes : colors.accent,
                fontSize: 9, fontWeight: 800, padding: '3px 7px', borderRadius: 4,
                border: `1px solid ${track.quality === 'HI-RES' ? colors.qualityHiRes : colors.accent}44`,
                letterSpacing: 0.5,
              }}>{track.quality}</span>
            )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          <button onClick={() => setShuffle(!shuffle)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, color: shuffle ? colors.primary : colors.textMuted }}>
            <Shuffle size={20} />
          </button>
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, color: colors.textPrimary }}>
            <SkipBack size={26} fill={colors.textPrimary} />
          </button>
          ...
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, color: colors.textPrimary }}>
            <SkipForward size={26} fill={colors.textPrimary} />
          </button>
          <button onClick={() => setRepeat(!repeat)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, color: repeat ? colors.primary : colors.textMuted }}>
            <Repeat size={20} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around' }}>
          {[
            { icon: <List size={20} />, label: 'Lyrics' },
            { icon: <Mic2 size={20} />, label: 'Karaoke' },
            { icon: <List size={20} />, label: 'Queue' },
            { icon: <Waves size={20} />, label: 'Spatial' },
          ].map(({ icon, label }) => (
